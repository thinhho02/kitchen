...
$date = $_POST['date'];
$select_receipts = mysqli_query($con, "SELECT `employees`.`employee_id`,`receipts`.`receipt_id`,`receipts`.`price`,`receipts`.`note`,`receipts`.`created_time` 
                                            FROM `employees` 
                                            INNER JOIN `payment` on `employees`.`employee_id` = `payment`.`employee_id`
                                            INNER JOIN `receipts` ON `payment`.`id` = `receipts`.`payment_id`
                                            WHERE `receipts`.`created_time` = '$date' and `receipts`.`status` = 'confirmed'");
