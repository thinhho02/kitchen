...
if(isset($_POST['date']) && $_POST['date'] !== ''){
    $date = $_POST['date'];
    $select_payment = mysqli_query($con,"SELECT SUM(`total`) as `total`,DATE_FORMAT(`created_time`,'%m/%Y') as `monthYear` FROM `payment` WHERE DATE_FORMAT(`created_time`,'%Y') = '$date' GROUP BY DATE_FORMAT(`created_time`,'%Y-%m') asc");
