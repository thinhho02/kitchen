...
if (
    ...
    $quantity_dish = $_POST['quantity']; 
    $payment_yearMonth = date('Y-m',strtotime($_POST['day']));
    // echo $payment_yearMonth;
        ...
        if (mysqli_num_rows($select_payment) > 0) {
            ...
            if (mysqli_num_rows($select_receipt) > 0) {
                ...
                if (mysqli_num_rows($select_dishDetail) > 0) {
                    ...
                    $quantity_detail = $row_detail['quantity'] + $quantity_dish;
                    ...
                    $insert_detail = mysqli_query($con, "INSERT INTO `receipt_detail`(`receipt_id`, `menu_id`, `quantity`, `price`) VALUES ('$id_receipt',$id_menu,$quantity_dish,$price_menu)");
                ...
                $price_receipt = $price_menu * $quantity_dish;
                if (mysqli_query($con, "INSERT INTO `receipts`(`receipt_id`, `payment_id`, `price`, `status`,`created_time`) VALUES ('$id_receiptTable',$id_payment,$price_receipt,'cart','$receipt_day')") === true) {
                    // insert vào id của receipts đã tạo trước đó vào bảng này
                    mysqli_query($con, "INSERT INTO `receipt_detail`(`receipt_id`, `menu_id`, `quantity`, `price`) VALUES ('$id_receiptTable',$id_menu,$quantity_dish,$price_receipt)");
            ...
            if (mysqli_query($con, "INSERT INTO `payment`(`employee_id`,`status`, `total`,`created_time`) VALUES ('$id_user',false,0,'$receipt_day')") === true) {
                ...
                if (mysqli_query($con, "INSERT INTO `receipts`(`receipt_id`, `payment_id`, `price`, `status`,`created_time`) VALUES ('$id2_receiptTable',$id2_payment,$price_menu,'cart','$receipt_day')") === true) {
                    // insert vào id của receipts đã tạo trước đó vào bảng này
                    mysqli_query($con, "INSERT INTO `receipt_detail`(`receipt_id`, `menu_id`, `quantity`, `price`) VALUES ('$id2_receiptTable',$id_menu,$quantity_dish,$price_menu)");
                    $data = ["message" => "thành công"];
                }
            }
        }
