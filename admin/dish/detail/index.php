            ...
            <div class="container pt-5">
                ...
                <div>
                    <div class="d-flex justify-content-center">
                        <img src="/kitchen/image/<?php echo $dish['image']; ?>" alt="dish"
                             class="img-thumbnail img-fluid" style="object-fit: contain; height: 20rem; width: 20rem;">
                    </div>
                ...
                <form id="updateForm" method="post" action="/kitchen/api/dish/updateDish.php">
                    <input name="id" hidden type="text" class="form-control" id="dish-id"
                           aria-label="dish id" value="<?= $dish['dish_id'] ?>">

                    <div class="form-group">
                        <label for="dish-name">Tên món ăn</label>
                        <input name="name" value="<?php echo $dish['name']; ?>" type="text" class="form-control"
                               id="dish-name" aria-describedby="dish name" placeholder="Nhập tên món ăn" required>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="dish-category" class="form-control" name="category_id">
                            ...
                            <?php foreach ($categories as $category): ?>
                                <option <?= ($category['id'] === $dish['category_id']) ? 'selected' : '' ?>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" required class="form-control" id="dish-description" rows="3"
                                  cols="50"><?= htmlspecialchars($dish['description']); ?></textarea>
                    </div>
                    ...
                    <table class="table table-striped table-bordered" id="resource-table">
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center">
                                        <button type="button" class="btn btn-link edit" aria-label="edit button"
                                                data-toggle="modal"
                                                data-id="<?= $detail['id'] ?>"
                                        ...
                                        <form id="deleteForm-<?= $detail['id'] ?>" method="post"
                                              action="/kitchen/api/dish/detail/deleteDishDetail.php">
                                            <input type="hidden" name="id" value="<?= $detail['id'] ?>">
                                            <button type="button" class="btn btn-link text-danger"
                                                    aria-label="delete button"
                                                    onclick="confirmDelete(<?php echo $detail['id'] ?>, '<?php echo $detail['name'] ?>', <?php echo $detail['quantity'] ?>)">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                        ...
                        <?php foreach ($resources as $key => $resource): ?>
                            <tr>
                                ...
                                <td class="text-center">
                                    <button type="button" class="btn btn-link insert"
                                            data-name="<?= $resource['name'] ?>"
                                            data-price="<?= $resource['price'] ?>"
                                            data-unit="<?= $resource['unit'] ?>"
                                            data-toggle="modal"
                                            data-target="#insertResourceToDishModal"
                                            data-resource-id="<?php echo $resource['id'] ?>">Thêm
                                    </button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
...
<div class="modal fade" id="insertResourceToDishModal" tabindex="-1" role="dialog" aria-labelledby="update modal"
     aria-hidden="true">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name-resource-insert" disabled
                               aria-describedby="name resource" placeholder="Nhập tên">
                    </div>
                    <div class="form-group">
                        <label for="unit-resource-insert">Đơn vị:</label>
                        <input type="text" class="form-control" id="unit-resource-insert" disabled
                               aria-describedby="unit resource" placeholder="Nhập đơn vị">
                    </div>
                    <div class="form-group">
                        <label for="price-resource-insert">Giá:</label>
                        <input type="text" class="form-control" id="price-resource-insert" disabled
                               aria-describedby="price resource" placeholder="Nhập giá">
                    </div>
                    <div class="form-group">
                        <label for="quantity-dish-detail">Số lượng:</label>
                        <input name="quantity" type="number" class="form-control" id="quantity-dish-detail"
                               aria-describedby="name resource" placeholder="Nhập số lượng" min="1" required value="1">
                    </div>
                </div>
...
<div class="modal fade" id="updateDishDetail" tabindex="-1" role="dialog" aria-labelledby="update modal"
     aria-hidden="true">
    <div class="modal-dialog" role="dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name-resource" disabled
                               aria-describedby="name resource" placeholder="Nhập tên">
                    </div>
                    <div class="form-group">
                        <label for="unit-resource">Đơn vị:</label>
                        <input type="text" class="form-control" id="unit-resource" disabled
                               aria-describedby="unit resource" placeholder="Nhập đơn vị">
                    </div>
                    <div class="form-group">
                        <label for="price-resource">Giá:</label>
                        <input type="text" class="form-control" id="price-resource" disabled
                               aria-describedby="price resource" placeholder="Nhập giá">
                    </div>
                    <div class="form-group">
                        <label for="update-quantity-dish-detail">Số lượng:</label>
                        <input name="quantity" type="number" class="form-control" id="update-quantity-dish-detail"
                               aria-describedby="quantity dish detail" placeholder="Nhập số lượng" required min="1">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Đóng</button>
